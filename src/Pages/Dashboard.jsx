...
const Dashboard = () => {
  ...
  useGSAP(
    ...
    { dependencies: [isLoading], scope: null, revertOnUpdate: true }
  );


  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-screen bg-[#0c164f]">
        <div className="animate-spin rounded-full h-20 w-20 border-8 border-t-[#ff00d4] border-gray-200"></div>
      </div>
    );
  }
  ...
  return (
    <>
      {isLoading ? (
        <div className="flex items-center justify-center h-screen bg-[#1d1135]">
          <div className="loader border-t-4 border-[#ba1e68] border-solid rounded-full w-16 h-16 animate-spin"></div>
        </div>
      ) : (
        <div>
          <Header />
